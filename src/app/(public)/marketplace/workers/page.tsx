...
import { Search, Star, BadgeCheck, MapPin, Filter, X } from "lucide-react";
import { searchWorkers } from "@/actions/marketplace";
...
const WORKER_TYPES: { value: WorkerType; label: string }[] = Object.entries(
  WORKER_TYPE_LABELS
).map(([value, label]) => ({ value: value as WorkerType, label }));
...
type WorkerResult = Awaited<ReturnType<typeof searchWorkers>>["workers"][number];
...
export default function BrowseWorkersPage() {
  ...
  // Filters
  const [selectedTypes, setSelectedTypes] = useState<WorkerType[]>([]);
  ...
  const fetchWorkers = useCallback(async () => {
    ...
    try {
      const result = await searchWorkers({
        workerType: selectedTypes.length > 0 ? selectedTypes : undefined,
        city: city || undefined,
        state: state || undefined,
        minScore: minScore > 0 ? minScore : undefined,
        credentialsVerified: credentialsVerified || undefined,
        sortBy,
      });
  ...
  }, [selectedTypes, city, state, minScore, credentialsVerified, sortBy]);

  useEffect(() => {
    fetchWorkers();
  }, [fetchWorkers]);

  function toggleWorkerType(type: WorkerType) {
  ...
  const hasActiveFilters =
    selectedTypes.length > 0 || city || state || minScore > 0 || credentialsVerified;
    ...
    <div className="relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white">Healthcare Workers</h1>
            <p className="text-sm text-slate-400 mt-1">
              {total} worker{total !== 1 ? "s" : ""} available on the marketplace
            </p>
          </div>
                  ...
                  {WORKER_TYPES.map((type) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => toggleWorkerType(type.value)}
                      ...
                      <div className="mt-4 flex items-center gap-4 text-xs text-slate-500">
                        ...
                        {worker.verifiedCredentialsCount > 0 && (
                          <span className="flex items-center gap-1">
                            ...
                            {worker.verifiedCredentialsCount}
                          </span>
                        )}
                      </div>
